...
import de.fu_berlin.inf.jtourbus.JTourBusStop;
...
public class TourBusRoutesView extends ViewPart {
			...
			fViewer
					.addSelectionChangedListener(new ISelectionChangedListener() {
						...
						public void selectionChanged(SelectionChangedEvent event) {
							...
							fActionNextStop.setEnabled(obj instanceof BusStop
	...
	private void makeActions() {
		...
		{ // Action for renaming tours
				...
				public void run() {
					...
					if (!selection.isEmpty()) {
						...
						InputDialog inputBox = new InputDialog(
								getSite().getShell(),
								"Rename tour",
								"Please enter the name of tour the selected stops are supposed to be on:",
						...
						if (inputBox.getReturnCode() == InputDialog.OK) {
							...
							for (Object selected : selection.toList()) {
								if (selected instanceof BusStop) {
									...
									for (BusStop bs : new ArrayList<BusStop>(
											fContentProvider.fTourPlan.routes
													.get(selected))) {
							...
							UpdateStopsOperation op = new UpdateStopsOperation(
									busStopsToWriteBack);
							op.run();
						}
					}
			...
			fRenameTourAction
					.setToolTipText("Rename a tour or move individual stops to other tour.");
		}
